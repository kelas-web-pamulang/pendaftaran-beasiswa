...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $kuotaTambah = intval(htmlspecialchars(trim($_POST['stok_tambah']))) ?? 0;
    $kuotaKurang = intval(htmlspecialchars(trim($_POST['stok_kurang']))) ?? 0;
    $kuota = intval(htmlspecialchars($_POST['kuota']));
    $tanggal_perbarui_data = date('Y-m-d H:i:s');

    // Fetch current stock
    $result = $db->select("beasiswa", ['AND id_beasiswa=' => $id_beasiswa]);
    $current_kuota = intval($result[0]['kuota']);
...
<body>
    <div class="container registration-container">
        <div class="registration-card">
            ...
            <form action="" method="post">
                ...
                <div class="form-group mb-3">
                    <label for="inpt_kuota">Kuota Saat ini</label>
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" pattern="[0-9]*" class="form-control" id="kurangiKuota" name="stok_kurang" placeholder="Masukkan angka kurangi kuota" value="0" title="Masukkan angka saja">
                </div>
                <div class="form-group mb-3">
                    <label for="tambahKuota">Tambah Kuota</label>
                    <input type="text" pattern="[0-9]*" class="form-control" id="tambahKuota" name="stok_tambah" placeholder="Masukkan angka tambah kuota" value="0" title="Masukkan angka saja">
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            document.getElementById('kurangiKuota').addEventListener('change', function() {
                const kurangiKuota = parseInt(this.value, 10);
                const currentKuota = <?php echo $beasiswa['kuota']; ?>;

                if (kurangiKuota > currentKuota) {
                    alert('Maaf, kuota yang ingin dikurangi melebihi jumlah kuota yang tersedia.');
                    this.value = 0; // Set nilai input kembali ke nilai kuota yang tersedia
                }
            });
        });
    </script>
